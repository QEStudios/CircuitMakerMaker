    ...
    if messageHasLink:
        ...
        if messageHasSave:
            ...
            if message.attachments[0].content_type in [
                "video/x-ms-wmv",
                "video/x-matroska",
            ]:  # Check if it's a video that needs to be re-encoded
                if file.size > 8_000_000:  # max 8mb
                    return
                print("VIDEO TO TRANSCODE FOUND!!!!!")
                replied_msg = await message.reply(
                    "Looks like you sent a video file which won't play correctly in discord!\nI'm transcoding it right now..."
                )
                ...
                file = discord.File(
                    fp="transcoded_video.mp4", filename="transcoded.mp4"
                )
                await replied_msg.edit(
                    "Looks like you sent a video file which won't play correctly in discord!\nI've transcoded it so it plays correctly:",
                    file=file,
                )
                return
